...
# auto-detects castle_defense_p1_v28.zip (the periodic checkpoint) and resumes from
...
if ($existingTrainingPid -and (Get-Process -Id $existingTrainingPid -ErrorAction SilentlyContinue)) {
    ...
    $checkpoint = Get-Item "$pyDir\castle_defense_p1_v28.zip" -ErrorAction SilentlyContinue
    if ($checkpoint) {
        Write-Output "Found checkpoint from $($checkpoint.LastWriteTime) -- training will resume from there."
    } else {
        Write-Output "No castle_defense_p1_v28.zip found -- this will warm-start fresh from castle_defense_p1_v25_bc instead."
    }
...
if ($existingBenchPid -and (Get-Process -Id $existingBenchPid -ErrorAction SilentlyContinue)) {
    ...
    $proc = Start-Process -FilePath "powershell.exe" -ArgumentList "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "$pyDir\benchmark_checkpoints.ps1", "-ModelTag", "v28", "-IntervalMinutes", "25", "-GamesPerCheck", "150" -WorkingDirectory $pyDir `
